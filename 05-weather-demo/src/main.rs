...
use wstd::http::{Body, Client, Method, Request, Response, StatusCode, Uri};
...
/// code.
async fn handle(req: Request<Body>) -> Result<String> {
...
fn get_query(req: &Request<Body>) -> Result<Query> {
    let uri = req.uri();
