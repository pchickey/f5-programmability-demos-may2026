...
async fn handle(req: Request<Body>) -> Result<String> {
    ...
    let results = location_results
        ...
        // Collect a Vec of Futures. Note, we cannot name the exact type that
        // impls Future which is collected here, because its a special type
        // generated by rustc from the `async` block above.
        .collect::<Vec<_>>()
        // TryJoin::try_join takes a Vec of Futures which have Output =
        // Result<Item, Error>, and gives a Future with Output =
